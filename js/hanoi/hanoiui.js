(function (root) {
  ...
  var HanoiUI = Hanoi.HanoiUI = function (game) {
    ...
    for (var i = 0; i < this.size; i ++ ) {
      for(var t = 0; t <= 2; t++ ) {
        ...
        $('#hanoi_' + t).prepend(new_placeholder)
      }
    }
  };
  ...
  HanoiUI.prototype.setClicks = function() {
    ...
    $('.tower').on("click", function(event){
      var start = parseInt($(this).attr('id').split("_")[1], 10);
      $(event.currentTarget).addClass("currentTower");

      $('.tower').off('click');

      $('.tower').on("click", function() {
        $(event.currentTarget).removeClass("currentTower");
        var end = parseInt($(this).attr('id').split("_")[1], 10);
  ...
  HanoiUI.prototype.render = function() {
    ...
    that.game.towers.forEach(function(tower, ti, tArr) {
